...
stopuser = {}
token ='7132276768:AAHDitEfkdepJjJJ4PEwrGTCHGM7lcYFQiM'
...
def start(message):
	...
	keyboard.add(contact_button)
	bot.send_message(chat_id=message.chat.id, text=f'''<b>🌀 𝗧𝗵𝗲𝘀𝗲 𝗔𝗿𝗲 𝗧𝗵𝗲 𝗕𝗼𝘁'𝗦 𝗖𝗼𝗺𝗺𝗮𝗻𝗱𝘀</b>
━━━━━━━━━━━━━━━━  
🌀 𝗕𝗿𝗮𝗶𝗻𝘁𝗿𝗲𝗲 𝗔𝘂𝘁𝗵  
<code>/chk</code> 𝗻𝘂𝗺𝗯𝗲𝗿|𝗺𝗺|𝘆𝘆|𝗰𝘃𝗰  
🌀 𝗦𝗧𝗔𝗧𝗨𝗦 𝗢𝗡𝗟𝗜𝗡𝗘
━━━━━━━━━━━━━━━━  
🌀 𝟯𝗗 𝗟𝗢𝗢𝗞𝗨𝗣  
...
#						last
					...
					cm1 = types.InlineKeyboardButton(f"• {str(cc)} •", callback_data='u8')
					status = types.InlineKeyboardButton(f"• 𝙎𝙏𝘼𝙏𝙐𝙎 ➜ {str(last)} •", callback_data='u8')
					cm3 = types.InlineKeyboardButton(f"• 𝘼𝙋𝙋𝙍𝙊𝙑𝙀𝘿 ✅ ➜ [ {str(live)} ] •", callback_data='x')
					ccn = types.InlineKeyboardButton(f"• 𝘾𝘾𝙉 ☑️ ➜ [ {str(ccnn)} ] •", callback_data='x')
					cm4 = types.InlineKeyboardButton(f"• 𝘿𝙀𝘾𝙇𝙄𝙉𝙀𝘿 ❌ ➜ [ {str(dd)} ] •", callback_data='x')
					risk = types.InlineKeyboardButton(f"• 𝙍𝙄𝙎𝙆 🏴‍☠️ ➜ [ {str(riskk)} ] •", callback_data='x')
					cm5 = types.InlineKeyboardButton(f"• 𝙏𝙊𝙏𝘼𝙇 👻 ➜ [ {str(total)} ] •", callback_data='x')
...
<b>𝗕𝗼𝘁 𝗕𝘆: @Dow993uf</b>'''
					...
					if "Funds" in last or 'Invalid postal' in last or 'avs' in last or 'added' in last or 'Duplicate' in last or 'Approved' in last or 'live' in last:
						...
						ccnn+=1
						bot.send_message(call.from_user.id, msg_cvv)
					else:
						dd += 1
...
𝗕𝗼𝘁 𝗕𝘆: @Dow993uf</b>'''
	...
	if "Funds" in last or 'Invalid postal' in last or 'avs' in last or 'added' in last or 'Duplicate' in last or 'Approved' in last:
		...
		bot.edit_message_text(chat_id=message.chat.id, message_id=ko, text=msgcc)
	else:
